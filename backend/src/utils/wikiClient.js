...
export async function wikiApiQuery(params, { fetchImpl = fetch, retries = 3 } = {}) {
...
export async function fetchWikiSummary(title, { fetchImpl = fetch, retries = 3 } = {}) {
  const encoded = encodeURIComponent(title.replace(/ /g, '_'));

  for (let attempt = 0; attempt <= retries; attempt += 1) {
    const res = await fetchImpl(`${WIKI_REST}/page/summary/${encoded}`, {
...
export async function fetchWikiWikitext(title, { fetchImpl = fetch } = {}) {
...
export async function searchWikiTitle(query, { fetchImpl = fetch, limit = 5 } = {}) {
  const data = await wikiApiQuery(
    {
      action: 'query',
      list: 'search',
