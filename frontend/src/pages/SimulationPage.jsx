...
import { simulationApi } from '../api/client.js';
...
export default function SimulationPage() {
  ...
  const fetchStatus = useCallback(() => simulationApi.status(), []);
  ...
  async function handleStep() {
    const liveStatus = await runAction(() => simulationApi.live());
    if (!liveStatus?.isLive) return;
    await sleep(LIVE_DELAY_MS);
    await runAction(() => simulationApi.finish());
  }
  ...
  async function handleAutoRun() {
    ...
    try {
      while (autoRunningRef.current) {
        const liveStatus = await simulationApi.live();
        await refresh();
        ...
        const finishedStatus = await simulationApi.finish();
        await refresh();
  ...
  async function handleReset() {
    autoRunningRef.current = false;
    setAutoRunning(false);
    await runAction(() => simulationApi.reset());
  }
